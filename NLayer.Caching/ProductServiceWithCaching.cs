...
namespace NLayer.Caching
{
    public class ProductServiceWithCaching : IProductService
    {
        ...
        {
            var product =  _memoryCache.Get<List<Product>>(CacheProductKey)?.FirstOrDefault(x => x.Id == id) ?? throw new NotFoundException($"{nameof(Product)} ({id}) not found");
        ...
        public Task<CustomResponseDto<List<ProductWithCategoryDto>>> GetProductsWithCategoryAsync()
        {
            ...
            var resultDto = _mapper.Map<List<ProductWithCategoryDto>>(result);

            return Task.FromResult(CustomResponseDto<List<ProductWithCategoryDto>>.Success(200, resultDto));
        }
